...
sgx_status_t add_form(sgx_enclave_id_t eid, sgx_status_t* retval, char* name, size_t len, char* origin, size_t origin_len, uint16_t x, uint16_t y, uint16_t width, uint16_t height)
{
	sgx_status_t status;
	ms_add_form_t ms;
	ms.ms_name = name;
...
sgx_status_t create_add_overlay_msg(sgx_enclave_id_t eid, uint8_t* output, uint32_t* out_len, char* input)
{
	sgx_status_t status;
	ms_create_add_overlay_msg_t ms;
	ms.ms_output = output;
	ms.ms_out_len = out_len;
	ms.ms_input = (char*)input;
	ms.ms_input_len = input ? strlen(input) + 1 : 0;
	status = sgx_ecall(eid, 17, &ocall_table_isv_enclave, &ms);
	return status;
}

sgx_status_t create_remove_overlay_msg(sgx_enclave_id_t eid, uint8_t* output, uint32_t* out_len, char* input)
{
	sgx_status_t status;
