...
sgx_status_t SGX_UBRIDGE(SGX_NOCONVENTION, enc_make_http_request, (const char* method, const char* url, const char* headers, const char* request_data, uint8_t* p_mac, int* ret_code));
void SGX_UBRIDGE(SGX_NOCONVENTION, ocall_write_file, (const char* fname, const char* data, size_t len));
size_t SGX_UBRIDGE(SGX_NOCONVENTION, ocall_get_file_size, (const char* fname));
void SGX_UBRIDGE(SGX_NOCONVENTION, ocall_read_file, (const char* fname, char* data, size_t len));
...
sgx_status_t test_decryption(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* form, uint32_t form_size, uint8_t* gcm_mac);
