...
static sgx_status_t SGX_CDECL sgx_add_form(void* pms)
{
	...
	ms->ms_retval = add_form(_in_name, _tmp_len, _in_origin, _tmp_origin_len, ms->ms_x, ms->ms_y, ms->ms_width, ms->ms_height);
err:
...
static sgx_status_t SGX_CDECL sgx_create_add_overlay_msg(void* pms)
{
	...
	size_t _len_input = ms->ms_input_len ;
	char* _in_input = NULL;
	...
	if (_tmp_input != NULL && _len_input != 0) {
		...
		_in_input[_len_input - 1] = '\0';
		if (_len_input != strlen(_in_input) + 1)
		{
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	create_add_overlay_msg(_in_output, _in_out_len, _in_input);
err:
	if (_in_output) {
...
static sgx_status_t SGX_CDECL sgx_create_remove_overlay_msg(void* pms)
{
	...
	create_remove_overlay_msg(_in_output, _in_out_len, _in_input);
err:
	if (_in_output) {
		memcpy(_tmp_output, _in_output, _len_output);
		free(_in_output);
...
SGX_EXTERNC const struct {
	size_t nr_ecall;
	struct {void* ecall_addr; uint8_t is_priv;} ecall_table[19];
} g_ecall_table = {
	19,
	{
