...
sgx_status_t add_form(char* name, size_t len, char* origin, size_t origin_len, uint16_t x, uint16_t y, uint16_t width, uint16_t height);
sgx_status_t add_input(char* name, size_t len1, char* input_i, size_t len2, uint8_t* sig_form, size_t sig_form_size, int validate);
...
void create_remove_overlay_msg(uint8_t* output, uint32_t* out_len, char* input);
