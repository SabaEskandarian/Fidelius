...
sgx_status_t run_js(char* code, size_t len, const uint8_t* code_sig, size_t len2);
sgx_status_t add_form(const char* name, size_t len, const char* origin, size_t origin_len, uint16_t x, uint16_t y);
sgx_status_t add_input(const char* form_name, size_t len_form, const char* input_name, size_t len_input, const uint8_t* sig_form, size_t sig_form_size, int validate, uint16_t x, uint16_t y, uint16_t height, uint16_t width);
...
sgx_status_t test_decryption(uint8_t* form, uint32_t form_size, uint8_t* gcm_mac);
void get_http_response(char* response, size_t len);
